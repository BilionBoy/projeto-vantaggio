...
class ATipoUsuariosController < ApplicationController
  ...
  def index
    ...
    @pagy, @a_tipo_usuarios = pagy(@q.result)
  end
  ...
  def create
    ...
    if @a_tipo_usuario.save
      redirect_to a_tipo_usuarios_path, notice: t('messages.created_successfully')
    else
  ...
  def update
    if @a_tipo_usuario.update(a_tipo_usuario_params)
      redirect_to a_tipo_usuarios_path, notice: t('messages.updated_successfully'), status: :see_other
    else
  ...
  def destroy
    if @a_tipo_usuario.destroy
      redirect_to a_tipo_usuarios_url, notice: t('messages.deleted_successfully')
    else
      redirect_to a_tipo_usuarios_url, alert: t('messages.delete_failed_due_to_dependencies')
    end   
  end
  ...
  def set_a_tipo_usuario
    ...
    return redirect_to a_tipo_usuarios_path, alert: t('messages.not_found') unless @a_tipo_usuario
  end
  ...
  def handle_not_found
    redirect_to a_tipo_usuarios_path, alert: t('messages.not_found')
  end
end
